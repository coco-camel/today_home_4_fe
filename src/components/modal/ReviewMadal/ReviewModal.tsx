...
const ReviewModal = () => {
  ...
  const { isLoading, isError, data} = useQuery({
    queryKey: ['getProductDetail', params.productId],
    queryFn: () => selectiveproduct(params.productId),
  ...
  const handleClickSubmit = () => {
    const productId: number = params?.productId ? Number(params.productId) : 0;
      reviewMutaion.mutate({ ...formData, productId });
    setFormData({
      contents: '',
      rating: 0,
    });
  ...
  return (
    <Fragment>
      ...
      <ModalTemplate $width={"720px"}>
        ...
        <ReviewProduct>
          <ImgBox>
            <img src={data?.product.imageUrl} alt="Product"/>
          </ImgBox>
          <FlexCloumBox>
            <span className={'brandname'}>{data?.product.brand}</span>
            <span className={'productname'}>{data?.product.name}</span>
          </FlexCloumBox>
        </ReviewProduct>
        <ModalBox>
          <span className={"spantext"}>별점 평가</span>
          <StarBox>
            <StarRating />
          </StarBox>
          <span className={'spantext'}>리뷰 작성</span>
          <textarea className={'reviewInput'} name="contents" placeholder={'자세하고 솔직한 리뷰는 다른 고객에게 큰 도움이 됩니다.'}
                    value={formData.contents} onChange={handleChange}/>
          <Sbmitbutton onClick={handleClickSubmit}>완료</Sbmitbutton>
        </ModalBox>
      </ModalTemplate>
    </Fragment>
  );
