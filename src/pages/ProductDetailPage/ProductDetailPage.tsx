...
import {
  ...
  FlexWrap,
  ProductSpan,
  ReivewWrap, ReviewBlock,
  ReviewBox, SpaceBtween
} from "./ProductDetailPage.styled";
  ...
  return (
    <>
      ...
      <Wrap>
        <Container>
          <Flex $width={'100%'}>
            <ContentBox $marginRight={'10px'} style={{flex: 1}}>
              <img src={data?.product.imageUrl} />
            </ContentBox>
            <ContentBox>
              <Flex className={'header'} $ac={true} $width={'100%'} $js={true}>
                <Flex className={'title'} $dc={true}>
                  <BrandName>{data?.product.brand}</BrandName>
                  <ProductSpan>{data?.product.name}</ProductSpan>
                </Flex>
                <button>찜</button>
              </Flex>
              ...
              <FlexWrap>
               <div className={'Delivery'}>배송</div>
                <div className={'DeliveryContent'}>무료배송</div>
               </FlexWrap>
                <BrandWrap>
                  <div>오디너리라이프</div>
                  <button>브랜드 홈</button>
      ...
      <ReivewWrap>
       <Container>
         <ReviewBox>
             ...
             <span className={"reviewrite"} onClick={handleClickShowModal}>리뷰쓰기</span>
         </ReviewBox>
         <ReviewBlock>
           <div>
             <div>프로필</div>
